...
# create another dataframe without variable Seychelles
...
From the table, we can see that the coefficient of interaction variable dropped from 0.34 to 0.25 which seems like the effect of ruggedness upon continent decreased.
...
If we drop the data of Seyhelles, the slope became slower in African nations which imply the effect of interaction variable is weaker than expected.
...
w <-
  w %>% mutate(ratio = w$waic / sum(w$waic) )
...
The result of the model-averaged predictions are even more easier to see the weak effect of interaction. The slope looks nearly flat.
...
If we includes area, the mean length of growing season was slightly reduced.
...
when controlling for area, the SD slope is reduced a lot in magnitude (-0.36 to 0.21)
...
The estimates table quite make sense for an interaction.
